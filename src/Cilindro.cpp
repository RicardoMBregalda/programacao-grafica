...
void Cilindro::generateVertices() {
    ...
    // V�rtices da borda da tampa superior
    for (int i = 0; i <= segments; ++i) {
        float theta = (float)i / segments * 2.0f * M_PI;
        float x = radius * cos(theta);
        float z = radius * sin(theta);
    ...
    // V�rtices da borda da tampa inferior
    for (int i = 0; i <= segments; ++i) {
        float theta = (float)i / segments * 2.0f * M_PI;
        float x = radius * cos(theta);
        float z = radius * sin(theta);
...
void Cilindro::draw(Shader &shader, glm::mat4 model) {
    ...
    // Se há uma textura específica para este cilindro, usa ela
    if (texture != nullptr) {
        texture->bind(0);
    }
